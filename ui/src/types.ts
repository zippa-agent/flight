...
export function parseContextLine(line: string): AwarenessEntry | null {
  try {
    ...
    const flightEntry = parseFlightAwarenessEntry(raw);
    if (flightEntry) return flightEntry;
...
function parseFlightAwarenessEntry(raw: unknown): AwarenessEntry | null {
  if (!isRecord(raw)) return null;
  if (typeof raw.id !== 'string' || typeof raw.timestamp !== 'string') return null;
  if (raw.type !== 'message' && raw.type !== 'tool_call' && raw.type !== 'tool_result') return null;

  const role = normalizeFlightRole(raw.role);
...
function normalizeFlightRole(value: unknown): AwarenessEntry['role'] | null {
  if (value === 'user' || value === 'assistant') return value;
  if (value === 'tool' || value === 'toolResult') return 'toolResult';
  return null;
}
